$(document).ready(function () {
    ...
    get_rs_edit_form(current_rowid);
});
...
function get_state_list(selected_value) {
	...
	$.ajax({
		type: "POST",
		url: BASE_URL + 'RSController/get_state_list',
		data: {},
		dataType: "json",
...
function get_division_list(state,selected_value) {
	if (state == '') {
		...
		$.ajax({
			type: "POST",
			url: BASE_URL + 'RSController/get_division_list',
			data: {
				"state": state,
			},
			dataType: "json",
...
function get_town_list(division,selected_value) {
	if (division == '') {
		...
		$.ajax({
			type: "POST",
			url: BASE_URL + 'RSController/get_town_list',
			data: {
				"division": division,
			},
			dataType: "json",
...
function get_additional_details(){
	...
	$.ajax({
		type: "POST",
		url: BASE_URL + 'RSController/get_additional_details_list_new',
		...
		success: function (data) {
			if (data.length != 0) {
				...
				var rs_html ='<option value="">Select</option>';
				...
				for (let index = 0; index < data.length; index++) {
					...
					else if(data[index].parameters =='Retailers Serviced'){
						rs_html +='<option value="' + data[index].slab + ' | ' + data[index].points + '">' + data[index].slab + '</option>';
					}
				...
				$('#c_retail_serviced').html(rs_html);
...
function get_rs_edit_form(current_rowid){
    $.ajax({
        type: "POST",
        url: BASE_URL + 'RSController/get_rs_edit_form',
        data: {
        ...
        success: function (data) {
            if (data.length != 0) {
                ...
                $('#c_ex_rs_name').val(data[0].c_ex_rs_name);
                ...
                $("#c_retail_serviced").val(data[0].c_retail_serviced+' | '+data[0].c_retail_serviced_point).trigger('change');
...
$('#rsForm').submit(function(e) {
	...
	if(c_name =='' || c_sname =='' || c_mobile_no =='' || c_state =='' || c_division =='' || c_town =='' || c_address ==''){
		...
		$.ajax({  
			url:BASE_URL + 'RSController/editRsForm', 
			method:"POST",  
			data: formData,
			cache:false,
			contentType: false,
