...
$("#key_name_list").on("change",function(){
  ...
  // console.log(startDate);
        $.ajax({
          type: "POST",
          url: BASE_URL + 'ss_recruitment/SSController/get_title_list',
          data: {
            kay_name: urlmenu,
            // startDate: startDate,
          },
          ...
          success: function (data) {
                ...
                var lengthOfArr = data.length;
                    ...
                    html +='<tr>';
                    html +='<td data-label="Parameter">Absenteeism (HoW & Total calls condition)</td>';
                    html +='<td data-label="Point">5</td>';
                    $.each(data, function (key, val) {
                        var values= val.key_absenteeism_point;
                        if (values==5) {
                    html +='<td data-label="Points" style="background-color: #90EE90;">'+val.key_absenteeism_point+'</td>'; }
                    else if (values ==3) { 
                        html +='<td data-label="Points" style="background-color: orange;">'+val.key_absenteeism_point+'</td>'; }
                        else if (values ==2){
                         html +='<td data-label="Points" style="background-color: red;">'+val.key_absenteeism_point+'</td>';
                        } else{
                            html +='<td data-label="Points" style="background-color: red;">'+val.key_absenteeism_point+'</td>';
                        }
                    values="";
                    ...
                    html +='<tr>';
                    html +='<td data-label="Parameter">Absenteeism (Actual Absent)</td>';
                    html +='<td data-label="Point">5</td>';
                    $.each(data, function (key, val) {
                         var values= val.key_absenteeism_actual_point;
                        if (values==5) {
                    html +='<td data-label="Points" style="background-color: #90EE90;">'+val.key_absenteeism_actual_point+'</td>'; }
                    else if (values ==3) { 
                        html +='<td data-label="Points" style="background-color: orange;">'+val.key_absenteeism_actual_point+'</td>'; }
                        else if (values ==2){
                         html +='<td data-label="Points" style="background-color: red;">'+val.key_absenteeism_actual_point+'</td>';
                        } else{
                            html +='<td data-label="Points" style="background-color: red;">'+val.key_absenteeism_actual_point+'</td>';
                        }
                    values="";
                ...
                $.each(data, function (key, val) {
                        // console.log(val.key_stocks_point);
                         total_points = Number(val.key_stocks_point) + Number(val.key_infra_point) + Number(val.key_infra_delivery_point) +  Number(val.key_number_point) + Number(val.key_order_point) + Number(val.key_absenteeism_point)  + 
                    Number(val.key_absenteeism_actual_point) + Number(val.key_npd_point) + Number(val.key_financials_point)  +  Number(val.key_infrastructure_point) + Number(val.key_ssfa_point)+ Number(val.key_xdm_point)+ Number(val.key_issues_raised_point);
