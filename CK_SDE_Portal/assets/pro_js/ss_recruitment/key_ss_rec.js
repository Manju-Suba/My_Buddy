...
function get_key_forms() {
    var example = $('#keyForm_tb').DataTable({
        ...
        'ajax': {
            'url': BASE_URL + 'ss_recruitment/SSController/get_key_ssforms',
            'data': function (data) {
    ...
    $('#keyForm_tb tbody').unbind('click');

    // Add event listener for opening and closing details
    $('#keyForm_tb tbody').on('click', 'td.details-control', function () {
...
function get_adtkeydetails_pop(table_row_id){
    ...
    $.ajax({
        type: "POST",
        url: BASE_URL + 'ss_recruitment/SSController/get_adtkeydetails_ss',
        data: {
        ...
        success: function (data) {
            if (data.length != 0) {
                ...
                var total_points = Number(data[0].key_stocks_point) + Number(data[0].key_infra_point) + Number(data[0].key_infra_delivery_point) +  Number(data[0].key_number_point) + Number(data[0].key_order_point) + Number(data[0].key_absenteeism_point)  + 
                    Number(data[0].key_absenteeism_actual_point) + Number(data[0].key_npd_point) + Number(data[0].key_financials_point)  +  Number(data[0].key_infrastructure_point) + Number(data[0].key_ssfa_point)+ Number(data[0].key_xdm_point)+ Number(data[0].key_issues_raised_point);
                ...
                html +='<td data-label="Slab">'+data[0].key_absenteeism+'</td>';
                html +='<td data-label="Points">'+data[0].key_absenteeism_point+'</td>';
                html +='</tr>';
                
                html +='<tr>';
                html +='<td data-label="Parameter">Absenteeism (Actual Absent)</td>';
                html +='<td data-label="Slab">'+data[0].key_absenteeism_actual+'</td>';
                html +='<td data-label="Points">'+data[0].key_absenteeism_actual_point	+'</td>';
                html +='</tr>';
