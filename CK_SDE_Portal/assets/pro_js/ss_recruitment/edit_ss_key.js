$(document).ready(function () {
    ...
    get_ss_key_edit_form(current_rowid);
});
...
function get_ss_key_edit_form(current_rowid){
    $.ajax({
        type: "POST",
        url: BASE_URL + 'ss_recruitment/SSController/get_ss_key_edit_form',
        data: {
        ...
        success: function (data) {
            if (data.length != 0) {
               $('#edit_row_id').val(current_rowid);
                ...
                $("#key_absenteeism").val(data[0].key_absenteeism+' | '+data[0].key_absenteeism_point).trigger('change');
                $("#key_absenteeism_actual").val(data[0].key_absenteeism_actual+' | '+data[0].key_absenteeism_actual_point).trigger('change');
...
$('#ssKeyForm').submit(function(e) {
    ...
    $.ajax({  
        url:BASE_URL + 'ss_recruitment/SSController/editKeySsForm', 
        method:"POST",  
        data: formData,
        cache:false,
        contentType: false,
