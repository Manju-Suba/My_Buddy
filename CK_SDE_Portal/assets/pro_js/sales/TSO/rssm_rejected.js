...
function get_rejected_forms() {
    ...
    var example = $('#rejectedForm_tb').DataTable({
        ...
        'ajax': {
            'url': BASE_URL + 'RSSMController/get_rejected_rssmforms',
            'data': function (data) { 
    ...
    $('#rejectedForm_tb tbody').unbind('click');

    // Add event listener for opening and closing details
    $('#rejectedForm_tb tbody').on('click', 'td.details-control', function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.get_adtdetails_rssm_sde_result.length != 0) {
                ...
                $('#sde_name').text(data.get_adtdetails_rssm_sde_result[0].tso);//business_division
