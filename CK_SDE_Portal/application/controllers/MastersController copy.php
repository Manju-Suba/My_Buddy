    ...
    public function osm_excel_upload(){
		...
		if(empty($error)){
			...
			try {
				...
				$value_exist = [];	
				$inserdata = [];	
				if($allDataInSheet[1]['B']=='osm_name' && $allDataInSheet[1]['C'] == 'ssfa_id' && $allDataInSheet[1]['D'] == 'sm_type' && $allDataInSheet[1]['E'] == 'zsm' && $allDataInSheet[1]['F'] == 'asm' && $allDataInSheet[1]['G'] == 'sde' && $allDataInSheet[1]['H'] == 'sde_id' && $allDataInSheet[1]['I'] == 'jc_type' && $allDataInSheet[1]['J'] == 'bc_target' && $allDataInSheet[1]['K'] == 'tlsd_target' && $allDataInSheet[1]['L'] == 'eco_target' && $allDataInSheet[1]['M'] == 'bc_achivement' && $allDataInSheet[1]['N'] == 'tlsd_achivement' && $allDataInSheet[1]['O'] == 'eco_achivement' && $allDataInSheet[1]['P'] == 'bc_percentage' && $allDataInSheet[1]['Q'] == 'tlsd_percentage' && $allDataInSheet[1]['R'] == 'eco_percentage'){		
					foreach ($allDataInSheet as $value) {					
						if($flag){
							$flag =false;
							continue;
			...
			try {
				...
				if($allDataInSheet[1]['B']=='division' && $allDataInSheet[1]['C'] == 'region' && $allDataInSheet[1]['D'] == 'zsm' && $allDataInSheet[1]['E'] == 'zsm_number' && $allDataInSheet[1]['F'] == 'zsm_email' && $allDataInSheet[1]['G'] == 'asm' && $allDataInSheet[1]['H'] == 'asm_number' && $allDataInSheet[1]['I'] == 'asm_email' && $allDataInSheet[1]['J'] == 'tso' && $allDataInSheet[1]['K'] == 'tso_number'  && $allDataInSheet[1]['L'] == 'tso_email' && $allDataInSheet[1]['M'] == 'sm' && $allDataInSheet[1]['N'] == 'sm_number'){			
				    foreach ($allDataInSheet as $value) {					
						...
						if($value['A'] !="" && $value['B'] !="" && $value['C'] !="" && $value['D'] !="" && $value['E'] !="" && $value['F'] !="" && $value['G'] !=""&& $value['H'] !="" && $value['I'] )					
						{		
							if(strlen($value['E']) == 10  && strlen($value['H']) == 10  && strlen($value['K']) == 10  && strlen($value['N']) == 10 && filter_var($value['F'], FILTER_VALIDATE_EMAIL) && filter_var($value['I'], FILTER_VALIDATE_EMAIL) &&filter_var($value['L'], FILTER_VALIDATE_EMAIL)  ){
								$password=md5("123456");
								$zsm_data=array("mobile"=>$value['E'],"username"=>$value['D'],"password"=>$password,"role"=>'ZSM',"role_type"=>'ZSM',"status"=>1,"business"=>$value['B']);
								$asm_data=array("mobile"=>$value['G'],"username"=>$value['F'],"password"=>$password,"role"=>'ASM',"role_type"=>'ASM',"status"=>1,"business"=>$value['B']);
								$tso_data=array("mobile"=>$value['I'],"username"=>$value['H'],"password"=>$password,"role"=>'TSO',"role_type"=>'TSO',"status"=>1,"business"=>$value['B']);
								$sm_data=array("mobile"=>$value['K'],"username"=>$value['J'],"password"=>$password,"role"=>'SM',"role_type"=>'SM',"status"=>1,"business"=>$value['B']);
								$zsm_user=$this->import->zsm_exist($value['E'],$zsm_data);
								$asm_user=$this->import->asm_exist($value['G'],$asm_data);
								$sm_user=$this->import->sm_exist($value['K'],$sm_data);
								$tso_user=$this->import->tso_exist($value['I'],$tso_data);
								$masters=$this->import->m_exist($value['E'],$value['G'],$value['I'],$value['K']);
								if($masters==1){
									if($beat_upload_type == "add"){
									    ...
									    $errors[]=['zsm_number' => $value['E'],'asm_number' => $value['G'],'sm_number' => $value['K'],'tso_number' => $value['I'],'format'=>'already_exists'];
									}
									else{
										$value_exist[$i]['zsm_number'] = $value['E'];
										$value_exist[$i]['asm_number'] = $value['G'];
										...
										$inserdata[$i]['asm_number'] = $value['G'];
										$inserdata[$i]['tso_number'] = $value['I'];
										$inserdata[$i]['sm_number']  = $value['K'];
										$inserdata[$i]['division'] = $value['B'];
										$inserdata[$i]['region'] = $value['C'];
										$inserdata[$i]['asm'] = $value['F'];
										$inserdata[$i]['tso'] = $value['H'];
										$inserdata[$i]['sm'] = $value['J'];
