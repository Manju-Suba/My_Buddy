				...
				if (strtolower($fileExtension) === 'csv') {
					...
					if (($handle = fopen($csvFile, 'r')) !== false) {
						...
						while (($row = fgetcsv($handle)) !== false) {
							...
							$allDataInSheet[$rowNumber] = array_combine(range('A', 'M'), $rowWithTrimmedSpaces);
							$rowNumber++; 
						}
						fclose($handle);
					}
				...
				if (strtolower($fileExtension) === 'csv') {
					...
					if (($handle = fopen($csvFile, 'r')) !== false) {
						...
						while (($row = fgetcsv($handle)) !== false) {
							...
							$allDataInSheet[$rowNumber] = array_combine(range('A', 'N'), $rowWithTrimmedSpaces);
							$rowNumber++; 
						}
						fclose($handle);
					}
				    ...
				    foreach ($allDataInSheet as $value) {					
						...
						{		
							if(strlen($value['E']) == 10  && strlen($value['H']) == 10  && strlen($value['K']) == 10  && strlen($value['N']) == 10   ){
								if(filter_var($value['F'], FILTER_VALIDATE_EMAIL) && filter_var($value['I'], FILTER_VALIDATE_EMAIL) &&filter_var($value['L'], FILTER_VALIDATE_EMAIL)){
									$password=md5("123456");
									$zsm_data=array("mobile"=>$value['E'],"username"=>$value['D'],"password"=>$password,"role"=>'ZSM',"role_type"=>'ZSM',"status"=>1,"business"=>$value['B']);
									$asm_data=array("mobile"=>$value['H'],"username"=>$value['G'],"password"=>$password,"role"=>'ASM',"role_type"=>'ASM',"status"=>1,"business"=>$value['B']);
									$tso_data=array("mobile"=>$value['K'],"username"=>$value['J'],"password"=>$password,"role"=>'TSO',"role_type"=>'TSO',"status"=>1,"business"=>$value['B']);
									$sm_data=array("mobile"=>$value['N'],"username"=>$value['M'],"password"=>$password,"role"=>'SM',"role_type"=>'SM',"status"=>1,"business"=>$value['B']);
