...
class MastersController extends CI_Controller {	
    ...
    public function rs_excel_upload(){
		...
		$config['remove_spaces'] = TRUE;
		$config['delimiter'] = "\t";
		$config['file_name'] = uniqid().'_'.$_FILES['rs_file']['name'];
		$this->load->library('upload', $config);
		$this->upload->initialize($config);   
			...
			try {
				...
				&&$allDataInSheet[1]['C']=='region' && $allDataInSheet[1]['D'] == 'state_name' && $allDataInSheet[1]['E'] == 'district_name' && $allDataInSheet[1]['F'] == 'city_name' && $allDataInSheet[1]['G'] == 'town_name' && $allDataInSheet[1]['H'] == 'town_code'){		
				    ...
				    foreach ($allDataInSheet as $value) {							
						...
						{
						if(strtoupper($value['B']) == 'FMCG URBAN' || strtoupper($value['B']) == 'AB EXCLUSIVE' || strtoupper($value['B']) == 'FMCG RURAL'
							 ||  strtoupper($value['B']) == 'SNACKS' || strtoupper($value['B']) == 'FMCG MT'){
							if(strlen($value['K'])== 10 && strlen($value['M'])== 10 && is_numeric($value['K']) && is_numeric($value['M'])){
								...
								$check_business_data=$this->import->check_sde_business($tso_mobile,$sm_mobile,strtoupper($value['B']));
								$check_sm_mapping=$this->import->check_sm_mapping($tso_mobile,$sm_mobile);	
								$check_sde_data=$this->import->check_sde_exists($tso_mobile,"tso_number");
								$check_sm_data=$this->import->check_sde_exists($sm_mobile,"sm_number");
								$check_cluster_details=$this->import->check_cluster_details($value['C'],$value['D']);
								// print_r($check_cluster_details);die;
								if ($check_cluster_details == 'cluster') {
									$errors[]=['rs_code' => $value['J'],'cluster' => $value['C'],'format'=>'cluster_er'];
								}elseif ($check_cluster_details == 'state_name') {
									$errors[]=['rs_code' => $value['J'],'statename' => $value['D'],'cluster' => $value['C'],'format'=>'state_er'];
								}elseif ($check_sm_data == 0) {
									$errors[]=['sm_mobile' => $value['K'],'format'=>'sm_not_found'];
								}elseif ($check_sde_data == 0) {
									$errors[]=['tso_mobile' => $value['M'],'format'=>'tso_not_found'];
								}elseif ($check_sm_mapping == 0) {
									$errors[]=['sm_mobile' => $value['K'],'tso_mobile' => $value['M'],'format'=>'mapping_not_found'];
								}elseif ($check_business_data == 0) {
									$errors[]=['sm_mobile' => $value['K'],'tso_mobile' => $value['M'],'business' =>$value['B'],'format'=>'business_mismatch'];
								}elseif($data==1){
									if($beat_upload_type == "add"){
									...
									$inserdata[$i]['business'] = strtoupper($value['B']);
    ...
    public function m_excel_upload(){
		...
		$config['allowed_types'] = 'xlsx|xls|csv';
		$config['remove_spaces'] = TRUE;
		$config['file_name'] = uniqid().'_'.$_FILES['m_file']['name'];
		$this->load->library('upload', $config);
		$this->upload->initialize($config);   
		...
		if(empty($error)){
			...
			try {
							 ...
							 ||  strtoupper($value['B']) == 'SNACKS' || strtoupper($value['B']) == 'FMCG MT'){
								...
								if(strlen($value['E']) == 10  && strlen($value['H']) == 10  && strlen($value['K']) == 10  && strlen($value['N']) == 10   ){
									if(filter_var($value['F'], FILTER_VALIDATE_EMAIL) && filter_var($value['I'], FILTER_VALIDATE_EMAIL) &&filter_var($value['L'], FILTER_VALIDATE_EMAIL)){ //&&filter_var($value['O'], FILTER_VALIDATE_EMAIL)
										$password=md5("123456");
										$zsm_data=array("mobile"=>$value['E'],"username"=>$value['D'],"email"=>$value['F'],"password"=>$password,"role"=>'ZSM',"role_type"=>'ZSM',"status"=>1,"business"=>strtoupper($value['B']));
										$asm_data=array("mobile"=>$value['H'],"username"=>$value['G'],"email"=>$value['I'],"password"=>$password,"role"=>'ASM',"role_type"=>'ASM',"status"=>1,"business"=>strtoupper($value['B']));
										$tso_data=array("mobile"=>$value['K'],"username"=>$value['J'],"email"=>$value['L'],"password"=>$password,"role"=>'TSO',"role_type"=>'TSO',"status"=>1,"business"=>strtoupper($value['B']));
										$sm_data=array("mobile"=>$value['N'],"username"=>$value['M'],"email"=>$value['O'],"password"=>$password,"role"=>'SM',"role_type"=>'SM',"status"=>1,"business"=>strtoupper($value['B']));
										
										$zsm_user=$this->import->zsm_exist($value['E'],$zsm_data,$beat_upload_type);
										$asm_user=$this->import->asm_exist($value['H'],$asm_data,$beat_upload_type);
										$sm_user=$this->import->sm_exist($value['N'],$sm_data,$beat_upload_type);
										$tso_user=$this->import->tso_exist($value['K'],$tso_data,$beat_upload_type);
										$masters=$this->import->m_exist($value['E'],$value['H'],$value['K'],$value['N']);
										$sm_mapping_exists=$this->import->sm_mapping_exist($value['N']);
										$sde_mapping_exists=$this->import->sde_mapping_exist($value['K'],$value['N'],$value['H']);
										$asm_mapping_exists=$this->import->asm_mapping_exist($value['H'],$value['K'],$value['E']);
										...
										// print_r($masters);
										// print_r($sm_mapping_exists);
										// print_r($sde_mapping_exists);
										// print_r($asm_mapping_exists);
										// die;
											if($masters==1 || $sm_mapping_exists == 1 || $sde_mapping_exists == 1 || $asm_mapping_exists ==1){
												if($beat_upload_type == "add"){
													...
													if($masters == 1){
														$errors[]=['zsm_number' => $value['E'],'asm_number' => $value['H'],'sm_number' => $value['N'],'tso_number' => $value['K'],'format'=>'already_exists'];
													}else
													if($sm_mapping_exists == 1){
														$errors[]=['sm_number' => $value['N'],'format'=>'already_exists',"type"=>"smMapping"];
													}else
													if($sde_mapping_exists == 1){
														$errors[]=['sm_number' => $value['N'],'tso_number' => $value['K'],'format'=>'already_exists',"type"=>"tsoMapping"];
													}else
													if($asm_mapping_exists == 1){
														$errors[]=['asm_number' => $value['H'],'sm_number' => $value['N'],'tso_number' => $value['K'],'format'=>'already_exists',"type"=>"asmMapping"];
													}
												}else if (( $sm_mapping_exists == 1 || $sde_mapping_exists == 1 || $asm_mapping_exists ==1) && $beat_upload_type == "over_write") {
													if( $sm_mapping_exists == 1){
													...
													$inserdata[$i]['division'] = strtoupper($value['B']);
													$inserdata[$i]['region'] = $value['C'];
				...
				if($beat_upload_type === "add"){
					...
					$result = $this->import->update_m_bysm($input,$value_exist,'sm');

				} else if ($sde_mapping_exists == 1 && $beat_upload_type == "over_write"){
					$result = $this->import->update_m_bysm($input,$value_exist,'sde');

				} else if ($asm_mapping_exists == 1 && $beat_upload_type == "over_write"){
					$result = $this->import->update_m_bysm($input,$value_exist,'asm');
	...
	public function get_role_based_user_list() {
		$role = $this->input->post('role');
		$fromlist = $this->import->get_table_user_list('masters',$role);
		echo json_encode($fromlist);
	}

	public function get_touser_list() {
		$role = $this->input->post('role');
		$from_person = $this->input->post('from_person');
		$fromlist = $this->import->get_touser_list('users', $role, $from_person);
		echo json_encode($fromlist);
	}

	public function replacement_user() {
		$role = $this->input->post('role');
		$from_person = $this->input->post('from_person');
		$to_person = $this->input->post('to_person');
		$division = $this->input->post('division');
		$replacement_action = $this->import->funct_replacement('masters', $role, $from_person, $to_person,$division);
		echo json_encode($replacement_action);
	}

	public function addNewUser() {
		if($this->input->post('newrole') == 'ZSM'){
			// $where_cond['business'] = $this->input->post('newbusiness');
			$ex_business = [];
				
			if (!is_array($this->input->post('newbusiness'))) {
				$decoded_business = json_decode($this->input->post('newbusiness'), true); 
				if (is_array($decoded_business)) {
					$ex_business = $decoded_business;
				} else {
					$ex_business = explode(',',$this->input->post('newbusiness'));
				}
			} else {
				$ex_business = $ex_business;
			}
			$where_cond['business'] =json_encode($ex_business);
			// print_r(json_encode($ex_business));
		}else{
			$where_cond['business'] = $this->input->post('newbusiness');
		}
		$where_cond['role'] = $this->input->post('newrole');
		$where_cond['role_type'] = $this->input->post('newrole');
		$where_cond['username'] = $this->input->post('newusername');
		$where_cond['mobile'] = $this->input->post('newmobile');
		$where_cond['email'] = $this->input->post('newemail');
		$where_cond['password'] = md5("123456");
		$where_cond['status'] = '1';

		$addform_result = $this->import->data_add('users',$where_cond,$where_cond['mobile']);

		if($addform_result){
			$result = array(
				"response" => $addform_result,
			);
		}
		else{
			$result = array(
	...
	public function get_division_list() {
		$role = $this->input->post('role');
		$from_person = $this->input->post('from_person');
		$fromlist = $this->import->get_div_list('users', $from_person,$role);
		echo json_encode($fromlist);
	}
}
