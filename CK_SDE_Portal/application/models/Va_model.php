...
class Va_model extends CI_Model{
    ...
    function verify_data_rsforms_va($postData,$postData_where,$postData_where_id){
		$response = array();
        //# Read value
		$draw = $postData['draw'];
		...
		$this->db->from('rs_recruitment_form  rrf');
		...
		foreach ($records as $record) {
            ...
            $action_url2 = base_url()."index.php/RSController/edit_rs_rec_form/".$record->auto_id;
			...
			$score 	= 	$record->c_age_of_org_point	 + $record->c_comp_handled_point + $record->c_retail_serviced_point + 
                        ...
                        $record->c_delivery_vehicle_point + $record->c_fut_inverstment_point + $record->c_prop_invol_point +
                        $record->c_market_fb_point;
						
			$action .= ' <a href="'.$action_url2.'"><button  class="btn  btn-sm btn-info  ml-1 dt-btn-st" id="editBtn"><i class="bx bx-pencil"></i></button></a>';
