...
public function verify_data_get($table,$where){
	...
	$this->db->join('masters as m','(rr.created_by = m.tso_number OR rr.created_by = m.asm_number OR rr.created_by = m.zsm_number)','left');
...
function list_menu_forms($where_cond,$postData_where,$postData){
	...
	public function get_asm_approved_forms_rssm($postData,$postData_where,$postData_where_id){
		...
		$this->db->join('masters as m', '(rrf.created_by = m.tso_number OR rrf.created_by = m.asm_number OR rrf.created_by = m.zsm_number)','left');
        $this->db->where($postData_where);
		if(count($postData_where_id) !=0){
			$this->db->where_in('rrf.created_by',$postData_where_id);
		}
        ...
        //# Fetch records
		$this->db->select('rrf.*,u.*,u.username as created_by_name');
		$this->db->from('rssm_recruitment_form rrf','left');
		$this->db->join('users as u', 'rrf.created_by = u.mobile','left');
		$this->db->join('masters as m', 'rrf.created_by = m.tso_number');
		// $this->db->join('masters as m','(rrf.created_by = m.tso_number OR rrf.created_by = m.asm_number OR rrf.created_by = m.zsm_number)','left');
		$this->db->where("(rrf.division_head_status = 'Approved' OR rrf.division_head_status IS NULL OR rrf.division_head_status = '')");
	...
	public function get_rssm_approved_forms_rssm($postData,$postData_where,$postData_where_id){
        ...
        //# Fetch records
		$this->db->select('rrf.*,m.*,u.*,u.username as created_by,u.mobile as createdBy_mobile');
		$this->db->from('rssm_recruitment_form rrf');
		$this->db->join('users as u', 'rrf.created_by = u.mobile','left');
		$this->db->join('masters as m', '(rrf.created_by = m.tso_number OR rrf.created_by = m.asm_number OR rrf.created_by = m.zsm_number)','left');
		...
		foreach ($records as $record) {
			...
			if($record->role == 'TSO'){
				$role = 'SDE';
				$sde = $record->tso;
				$sdemobile = $record->tso_number;
				$asm = $record->asm;
				$asmmobile = $record->asm_number;
				$zsm = $record->zsm;
				$zsmmobile = $record->zsm_number;
			}else if($role= $record->role == 'ASM'){
				$sde = '-';
				$sdemobile = '-';
				$asm = '-';
				$asmmobile = '-';
